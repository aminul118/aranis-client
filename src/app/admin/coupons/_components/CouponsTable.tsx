...
import { deleteCouponBulk } from '@/services/coupon/coupon';
      ...
      {selectedIds.length > 0 && (
        <div className="animate-in slide-in-from-top-4 flex items-center justify-between rounded-xl bg-blue-600 p-4 text-white shadow-xl duration-300">
          ...
          <div className="flex gap-2">
            <DeleteConfirmation
              onConfirm={() => deleteCouponBulk(selectedIds)}
              onSuccess={() => setSelectedIds([])}
              title="Delete Selected Coupons?"
              description={`Are you sure you want to delete ${selectedIds.length} coupons? This action cannot be undone.`}
            >
